...
- `클로드 <message>` routes to Claude Code.
- `코덱스 <message>` routes to Codex.
- `클로`, `덱스`, or any names in `CLAUDE_ALIASES` and `CODEX_ALIASES` also work.
...
- Prefix a request with `업무` for the local workspace context.
...
- Use `코덱스 카카오 나에게 보내기 할 일 정리해줘` to route Kakao self-room intake through the local workspace.
- Use `둘이 토론해`, `다음`, and `계속` for Debate Mode. Pure continuations only continue an active debate.
- `리셋 클로드` resets Claude channel session.
- `리셋 코덱스` resets Codex channel session.
- `리셋 토론` clears Debate Mode state.
- `!help` prints the command list in Slack.

## Context Guard

Slack is the control surface and the local workspace is the operating folder. Ordinary Slack chat is not automatically stored as workspace input. Explicit workspace actions such as Kakao, SMS, inbox, file, task, schedule, save, capture, archive, and register requests are classified before Debate Mode, so stale debate context cannot swallow a new direct command.
