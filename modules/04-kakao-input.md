...
Use Kakao as another input source, then route collected text into the local workspace instead of treating Kakao as a separate operating system.
...
2. Save raw records under `ledger/kakao`.
3. Save input markdown under `inputs/kakao`.
...
Slack is the command surface. Kakao is an input source. The local workspace is the operating folder.
